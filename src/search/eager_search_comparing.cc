#include "eager_search_comparing.h"
...
EagerSearchComparing::EagerSearchComparing(const Options &opts) :
		SearchEngine(opts), reopen_closed_nodes(
				opts.get<bool>("reopen_closed")), use_multi_path_dependence(
				opts.get<bool>("mpd")), open_list(
				opts.get<OpenList<StateID> *>("open")), f_evaluator(
				opts.get<ScalarEvaluator *>("f_eval", nullptr)), preferred_operator_heuristics(
				opts.get_list<Heuristic *>("preferred")) {
}

void EagerSearchComparing::initialize() {
	...
	Heuristic* heuristic;
	for (size_t h = 0; h < compare_evals.size(); ++h) {
		heuristic_values.push_back(std::vector<double>());
		heuristic = dynamic_cast<Heuristic*>(compare_evals[h]);
		heuristic->initialize_public(); // Manual because not using scalar evaluator
		compare_heuristics.push_back(heuristic);
	}
	...
	compute_heuristic_values(initial_state);
...
void EagerSearchComparing::print_checkpoint_line(int g) const {
	cout << "[g=" << g << ", ";
	statistics.print_basic_statistics();
	cout << "]" << endl;
}

void EagerSearchComparing::print_statistics() const {
	statistics.print_detailed_statistics();
	search_space.print_statistics();
}

SearchStatus EagerSearchComparing::step() {
	...
	if (check_goal_and_set_plan(s)) {
		...
		for (size_t i = 0; i < heuristic_values[0].size(); ++i) {
			for (size_t h = 0; h < heuristic_values.size(); ++h) {
				cout << heuristic_values[h][i] << "\t";
			}
			cout << endl;
		}
		return SOLVED;
	...
	for (const GlobalOperator *op : applicable_ops) {
		...
		if (succ_node.is_new()) {
			...
			EvaluationContext eval_context(succ_state, succ_g, is_preferred,
					&statistics);
			compute_heuristic_values(succ_state);
			statistics.inc_evaluated_states();
...
//			g_explored_states.insert(s);
			// Update heuristics (novelty)
			for (Heuristic *heuristic : heuristics) {
				heuristic->reached_state(s, *op, succ_state);
			}
...
pair<SearchNode, bool> EagerSearchComparing::fetch_next_node() {
...
void EagerSearchComparing::reward_progress() {
	// Boost the "preferred operator" open lists somewhat whenever
	// one of the heuristics finds a state with a new best h value.
	open_list->boost_preferred();
}

void EagerSearchComparing::dump_search_space() const {
	search_space.dump();
}

void EagerSearchComparing::start_f_value_statistics(
...
void EagerSearchComparing::update_f_value_statistics(const SearchNode &node) {
	if (f_evaluator) {
		/*
		 TODO: This code doesn't fit the idea of supporting
		 an arbitrary f evaluator.
		 */
		EvaluationContext eval_context(node.get_state(), node.get_g(), false,
				&statistics);
		int f_value = eval_context.get_heuristic_value(f_evaluator);
		statistics.report_f_value_progress(f_value);
	}
...
void EagerSearchComparing::compute_heuristic_values(const GlobalState& state) {
	Heuristic* heuristic;
	for (size_t h = 0; h < compare_heuristics.size(); ++h) {
		heuristic = compare_heuristics[h];
//		EvaluationResult result = eval->compute_result(eval_context);
		heuristic_values[h].push_back(
				heuristic->compute_heuristic_double(state));
	}
}
...
//
//	EagerSearchComparing *engine = nullptr;
//	if (!parser.dry_run()) {
//		opts.set<bool>("mpd", false);
//		engine = new EagerSearchComparing(opts);
//	}
//
//	return engine;
//}
...
static SearchEngine *_parse_astar(OptionParser &parser) {
	parser.document_synopsis("A* search (eager)",
	...
	parser.document_note("Equivalent statements using general eager search",
			"\n```\n--search astar(evaluator)\n```\n"
					...
					"```\n", true);
	parser.add_list_option<ScalarEvaluator *>("evals", "evaluator for h-value");
	...
	if (!parser.dry_run()) {
		...
		EagerSearchComparing::compare_evals = opts.get_list<ScalarEvaluator *>(
				"evals");
		ScalarEvaluator *eval = EagerSearchComparing::compare_evals[0];
...
//	parser.document_note("Open list",
...
//					inner_lists.push_back(
//							new StandardScalarOpenList<StateID>(evaluator,
//									true));
//				}
//			}
...
std::vector<ScalarEvaluator*> EagerSearchComparing::compare_evals = std::vector<
		ScalarEvaluator*>();
//static Plugin<SearchEngine> _plugin("eager_cmp", _parse);
static Plugin<SearchEngine> _plugin_astar("astar_cmp", _parse_astar);
//static Plugin<SearchEngine> _plugin_greedy("eager_greedy_cmp", _parse_greedy);
