#ifndef ENFORCED_HILL_CLIMBING_SEARCH_DUPLICATE_DETECTION_H
#define ENFORCED_HILL_CLIMBING_SEARCH_DUPLICATE_DETECTION_H
...
typedef std::pair<StateID, std::pair<int, std::vector<const GlobalOperator * >>> OpenListEntryEHC;
...
*/
class EnforcedHillClimbingSearchDuplicateDetection : public SearchEngine {
    std::vector<const GlobalOperator *> get_successors(
        EvaluationContext &eval_context);
    void expand(EvaluationContext &eval_context, int d, std::vector<const GlobalOperator *> ops);
    void reach_state(
...
public:
    explicit EnforcedHillClimbingSearchDuplicateDetection(const Options &opts);
    virtual ~EnforcedHillClimbingSearchDuplicateDetection() override;
