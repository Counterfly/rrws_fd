...
EnforcedHillClimbingSearchRW::EnforcedHillClimbingSearchRW(
    const Options &opts)
    : SearchEngine(opts),
...
void EnforcedHillClimbingSearchRW::reach_state(
...
void EnforcedHillClimbingSearchRW::save_plan_if_necessary() const {
...
void EnforcedHillClimbingSearchRW::initialize() {
    assert(heuristic);
...
vector<const GlobalOperator *> EnforcedHillClimbingSearchRW::get_successors(
...
vector<const GlobalOperator *> EnforcedHillClimbingSearchRW::get_biased_successors(
    EvaluationContext &eval_context) {
    vector<const GlobalOperator *> ops;
	g_successor_generator->generate_applicable_ops(eval_context.get_state(), ops);
...
void EnforcedHillClimbingSearchRW::expand(EvaluationContext &eval_context, int d) {
...
bool EnforcedHillClimbingSearchRW::check_goal_and_set_plan(const GlobalState &state) {
    if (test_goal(state)) {
        ...
        StateID current_state_id = g_initial_state().get_id();
        // Plan = vector<const GlobalOperator *>
        cout << current_state_id;
        while (!(current_state_id == state.get_id())) {
        	vector<const GlobalOperator*> ops = used_actions.at(current_state_id);
        	for (const GlobalOperator* op : ops) {
        		plan.push_back(op);
        	}
        	current_state_id = next_state[current_state_id];
        	if (ops.size() > 1) {
        		//cout << "(" << ops.size() << ")";
        	}
        	//cout << " -> " << current_state_id;
...
void EnforcedHillClimbingSearchRW::search() {
	for (int i = 0; i < num_execution_samples; ++i) {
		initialize();
...
SearchStatus EnforcedHillClimbingSearchRW::step() {
...
SearchStatus EnforcedHillClimbingSearchRW::ehc() {
    while (!open_list->empty()) {
        ...
        if (node.is_new()) {
            ...
            if (h < current_eval_context.get_heuristic_value(heuristic)) {	// if hvalue of successor < hvalue of parent
                ...
                next_state[parent_state.get_id()] = state.get_id();
    ...
    cout << "Entering RWs: to beat: " << hvalue << endl;
    //int num_restarts = 1;
    ...
    do {
    	...
    	walk.clear();
    	actions.clear();
    	//walk.push_back(eval_context.get_state());
...
long EnforcedHillClimbingSearchRW::luby_sequence(long sequence_number) {
	long focus = 2L;
...
void EnforcedHillClimbingSearchRW::print_statistics() const {
	for (int i = 0; i < num_execution_samples; ++i) {
...
static SearchEngine *_parse(OptionParser &parser) {
    ...
    if (parser.dry_run())
        ...
        return new EnforcedHillClimbingSearchRW(opts);
}

static Plugin<SearchEngine> _plugin("ehc_rw", _parse);
