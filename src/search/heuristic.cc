...
Heuristic::Heuristic(const Options &opts) :
		description(opts.get_unparsed_config()), initialized(false), heuristic_cache(
				HEntry(NO_VALUE, true)), //TODO: is true really a good idea here?
		cache_h_values(opts.get<bool>("cache_estimates")), task(
				get_task_from_options(opts)), task_proxy(*task), cost_type(
...
void Heuristic::reached_state(const GlobalState & /*parent_state*/,
		const GlobalOperator & /*op*/, const GlobalState & /*state*/) {
...
static PluginTypePlugin<Heuristic> _type_plugin("Heuristic",
