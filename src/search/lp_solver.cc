...
void add_lp_solver_option_to_parser(OptionParser &parser) {
	...
	parser.add_enum_option("lpsolver", lp_solvers,
			"external solver that should be used to solve linear programs",
			"CPLEX", lp_solvers_doc);
}
...
LPVariable::LPVariable(double lower_bound_, double upper_bound_,
		...
		lower_bound(lower_bound_), upper_bound(upper_bound_), objective_coefficient(
				objective_coefficient_), is_integer(is_integer_) {
}
...
//	bool a = !lp_solver->isProvenPrimalInfeasible();
//	bool b = !lp_solver->isProvenDualInfeasible();
//	bool c = lp_solver->isProvenOptimal();
//	cout << "has optimal, " << is_solved << " " << a << " " << b << " " << c << endl;
	assert(is_solved);
	try {
...
//	cout << "lpsolver.extract solution" << endl;
//	bool a = !lp_solver->isProvenPrimalInfeasible();
//	bool b = !lp_solver->isProvenDualInfeasible();
//	bool c = lp_solver->isProvenOptimal();
//	cout << "extract=" << is_solved << " " << a << " " << b << " " << c << endl;
	assert(has_optimal_solution());
	try {
