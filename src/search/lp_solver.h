...
struct LPVariable {
	...
	LPVariable(double lower_bound_, double upper_bound_,
			double objective_coefficient_, bool is_integer_ = false);
	~LPVariable();
};
...
public:
	...
	LP_METHOD(void load_problem(
					...
					const std::vector<LPConstraint> &constraints))LP_METHOD(void add_temporary_constraints(const std::vector<LPConstraint> &constraints))LP_METHOD(void clear_temporary_constraints())LP_METHOD(double get_infinity() const)
	...
	LP_METHOD(void solve())LP_METHOD(void solve_mip())

	LP_METHOD(bool has_solution() const)
	...
	LP_METHOD(int get_num_variables() const)LP_METHOD(int get_num_constraints() const)LP_METHOD(int has_temporary_constraints() const)LP_METHOD(void print_statistics() const)
};
#ifdef __GNUG__
