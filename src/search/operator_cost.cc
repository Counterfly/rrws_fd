...
std::ostream& operator<<(std::ostream& out, const OperatorCost cost){
    static std::map<OperatorCost, std::string> strings;
    if (strings.size() == 0){
#define INSERT_ELEMENT(p) strings[p] = #p
        INSERT_ELEMENT(OperatorCost::NORMAL);
        INSERT_ELEMENT(OperatorCost::ONE);
        INSERT_ELEMENT(OperatorCost::PLUSONE);
        INSERT_ELEMENT(OperatorCost::MAX_OPERATOR_COST);
#undef INSERT_ELEMENT
    }

    return out << strings[cost];
}
