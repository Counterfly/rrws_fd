...
SearchEngine::SearchEngine(const Options &opts) :
		status(IN_PROGRESS), solution_found(false), search_space(
				OperatorCost(opts.get_enum("cost_type"))), cost_type(
				OperatorCost(opts.get_enum("cost_type"))), max_time(
...
double SearchEngine::get_probability() const {
...
void SearchEngine::set_plan_manually(const GlobalState &state) {
	Plan plan;
...
std::string SearchEngine::print_cost_type() const {
	stringstream ss;
	ss << cost_type;
	return ss.str();
}

static PluginTypePlugin<SearchEngine> _type_plugin("SearchEngine",
