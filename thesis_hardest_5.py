...
new_suite = []
for suite in SUITE:
    hardest_instances = helpers.get_last_problem_instances(suite, 5)
    new_suite.extend(hardest_instances)



file_name = os.path.splitext(os.path.basename(__file__))[0]
#STORE_PATH = "./data/" +SUITE[0] + '/' + file_name + "4"
STORE_PATH = './data_5_hardest/thesis'
...
class FastDownwardExperiment(object):
  ...
  def __init__(self):
    ...
    self._pool = Pool()	# Use 2 processes!! not cores.
  ...
  def run_benchmarks_single_algorithm(self, suite, algorithm, store_path):
	...
	pass
      ...
      #######
      # TODO: this is temporary to only run on last 5 instances, remove this
      # BEGIN REMOVE SECTION
      import helpers
      benchmark_instances = helpers.natural_sort(benchmark_instances)[-5:]
      # END REMOVE SECTION
      ######
...
# HA Bias 0.9
exp.add_algorithm('EHC_BFS_DD_PREF_RANKED', REPO, REV, ['--heuristic', 'hff=ff()', '--search', 'ehc(hff, preferred=[hff], preferred_usage=RANK_PREFERRED_FIRST)'])
exp.add_algorithm('EHC_LUBY_PREF0.9', REPO, REV, ['--heuristic', 'hff=ff()', '--search', 'ehc_rw(h=hff, restart=luby, preferred=[hff], pref_prob=0.9)'])
exp.add_algorithm('EHC_EXP_PREF0.9', REPO, REV, ['--heuristic', 'hff=ff()', '--search', 'ehc_rw(h=hff, restart=exp, preferred=[hff], pref_prob=0.9)'])
exp.add_algorithm('EHC_FP0.01_PREF0.9', REPO, REV, ['--heuristic', 'hff=ff()', '--search', 'ehc_rw_fixed_prob(h=hff, prob=0.01, preferred=[hff], pref_prob=0.9)'])
